...
export function RemoteWindowCollisionApp() {
  const [input, setInput] = React.useState({
    type: 'set-context',
    payload: {
      message: 'waiting shared window event',
      tag: 'collision-none',
    },
  });
  ...
  const emitAck = () => {
    emitToHost({
      type: 'acknowledged',
      payload: {
        source: 'react-window-collision-remote',
  ...
  return (
    <div style={{ border: '1px solid #7c3aed', borderRadius: '10px', background: '#faf5ff', padding: '12px' }}>
      <h4 style={{ marginTop: 0 }}>React Collision Remote (Window Event)</h4>
      <p style={{ margin: '6px 0' }}>Shared input type: <strong>{String(input?.type ?? '')}</strong></p>
      <p style={{ margin: '6px 0' }}>Shared input message: <strong>{String(input?.payload?.message ?? '')}</strong></p>
      <p style={{ margin: '6px 0' }}>Shared input tag: <strong>{String(input?.payload?.tag ?? '')}</strong></p>
