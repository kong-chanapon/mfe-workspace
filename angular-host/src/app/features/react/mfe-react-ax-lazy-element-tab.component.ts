...
type ReactAxElement = HTMLElement & {
  input?: RemoteInput;
  refresh?: () => void;
};

@Component({
  selector: 'app-mfe-react-ax-lazy-element-tab',
  imports: [LazyElementsModule],
  templateUrl: './mfe-react-ax-lazy-element-tab.component.html',
  styleUrl: './mfe-react-ax-lazy-element-tab.component.css',
  schemas: [CUSTOM_ELEMENTS_SCHEMA],
})
export class MfeReactAxLazyElementTabComponent {
  ...
  private remoteElementRef?: ElementRef<ReactAxElement>;

  private readonly config = getAppConfig();
  readonly remoteUrl = this.config.remotes.reactAxElementEntry;

  remoteInput: RemoteInput = {
    type: 'set-context',
    payload: {
      message: 'hello from angular host (react + axLazyElement)',
      tag: 'angular-to-react-ax',
    },
  };
  ...
  onLazyLoaded = (): void => {
    this.status = 'Loaded React micro by *axLazyElement';
  };

  onLazyError = (_error: ErrorEvent): void => {
    this.status = 'Failed to load React micro script by *axLazyElement';
  };
